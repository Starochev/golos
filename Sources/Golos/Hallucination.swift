...
enum Hallucination {
    ...
    /// Короче этого низкая плотность ни о чём не говорит: «Да.» за четыре
    /// секунды это нормальный ответ, а не потеря.
    private static let thinMinimumSeconds: TimeInterval = 6
