...
final class Controller {
    ...
    private func transcribe(wav: Data, generation: Int, attempt: Int,
        ...
        whisper.transcribe(wav: wav, prompt: prompt) { [weak self] result in
            ...
            case .success(let raw):
                ...
                // Пусто в двух случаях: весь ответ был выдумкой и вычистился,
                // либо движок не вернул вообще ничего. Под пустотой всё равно
                // лежит записанная речь, и молча терять её нельзя: так
                // пятнадцать секунд диктовки уходили в никуда.
                //
                // Раньше здесь стояло ещё и `!raw.isEmpty`, и пустой ответ
                // движка второй попытки не получал: диктовка умирала, а в
                // журнал шло «распознать не вышло». Замер на двух таких
                // записях: со второй попытки обе дают текст.
                if attempt == 1, cleaned.isEmpty {
                    let what = raw.isEmpty ? "движок вернул пустоту" : "кусок вычистился в ноль: «\(raw)»"
                    Log.write("\(what) — переспрашиваю без словаря")
                    self.transcribe(wav: wav, generation: generation, attempt: 2,
                                    completion: completion)
                    return
                }
    ...
    private func saveHistory(wav: Data, text: String) {
        ...
        // Пережимать сразу нельзя: свежая запись нужна нетронутой, по ней
        // разбираются жалобы на распознавание, а m4a теряет часть звука.
        // Полежит полчаса и пережмётся вместе с остальными.
        History.compressLeftovers()
    }
